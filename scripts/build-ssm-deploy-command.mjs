...
const deployScript = [
  ...
  '  if curl -fsSLk http://127.0.0.1/ | grep -F "app-build-sha" | grep -F "${DEPLOY_SHA}" > /dev/null; then',
  '    echo "Frontend serving deployed SHA ${DEPLOY_SHA} over HTTP/HTTPS."',
  '    break',
  '  fi',
  '  if [ "${attempt}" -eq 18 ]; then',
  '    echo "Frontend did not serve deployed SHA ${DEPLOY_SHA} over HTTP/HTTPS after deploy."',
  '    curl -fsSLk http://127.0.0.1/ || true',
  '    exit 1',
  '  fi',
  '  sleep 5',
  'done',
  'for attempt in $(seq 1 18); do',
  '  if curl -fsSLk http://127.0.0.1/api/health > /dev/null; then',
  '    echo "API respondeu health check em HTTP/HTTPS."',
  '    break',
  '  fi',
  '  if [ "${attempt}" -eq 18 ]; then',
  '    echo "API nao respondeu o health check em HTTP/HTTPS a tempo."',
  '    curl -fsSLk http://127.0.0.1/api/health || true',
  '    exit 1',
  '  fi',
  '  sleep 5',
  'done',
  ...
  'docker compose run --rm certbot certonly --preferred-profile shortlived --webroot --webroot-path /var/www/certbot --non-interactive --agree-tos --cert-name "${APP_CERT_NAME}" "${CERTBOT_RENEW_ARGS[@]}" "${CERTBOT_CONTACT_ARGS[@]}" -d "${APP_HOST}"',
