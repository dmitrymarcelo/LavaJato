...
const deployScript = [
  ...
  '  if curl -fsS --resolve "${APP_DOMAIN}:80:127.0.0.1" "http://${APP_DOMAIN}/" | grep -F "app-build-sha" | grep -F "${DEPLOY_SHA}" > /dev/null; then',
  ...
  '    curl -fsS --resolve "${APP_DOMAIN}:80:127.0.0.1" "http://${APP_DOMAIN}/" || true',
  '    exit 1',
  '  fi',
  '  sleep 5',
  'done',
  'curl -fsS --resolve "${APP_DOMAIN}:80:127.0.0.1" "http://${APP_DOMAIN}/api/health"',
