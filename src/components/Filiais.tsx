  ...
  return (
    <div className="flex flex-col min-h-full bg-white pb-24">
      <div className="grid grid-cols-2 gap-3 p-4 pb-2">
        ...
        <div className="metric-card bg-slate-50 border border-slate-100 p-4 rounded-2xl">
          <div className="flex items-center justify-between mb-2">
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-wider">Atendimentos (Mes)</p>
            <Car className="text-slate-500 w-4 h-4" />
          </div>
          <p className="text-2xl font-black text-slate-900 leading-tight">{bases.reduce((total, base) => total + base.vehicles, 0)}</p>
          <div className="flex items-center gap-1 mt-1">
            <span className="text-emerald-500 text-[10px] font-bold uppercase">Capacidade somada</span>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="bg-slate-50 rounded-xl p-3">
              ...
              <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${
                    base.status === 'active' ? 'bg-emerald-500' : base.status === 'warning' ? 'bg-amber-500' : 'bg-rose-500'
                  }`}
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                <span className={`text-[10px] font-bold ${base.status === 'critical' ? 'text-rose-500' : 'text-slate-500'}`}>
                  {base.status === 'critical' ? 'Abaixo da meta' : `${Math.round((Number(base.spent.replace('.', '')) / Number(base.budget.replace('.', ''))) * 100)}% concluido`}
                </span>
              </div>
            </div>
          </div>
