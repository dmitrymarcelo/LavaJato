...
import { CheckCircle2, Clock, ChevronRight, CreditCard, Wallet } from 'lucide-react';
import { Screen } from '../types';

export default function Payment({ onNavigate, onPaymentComplete }: { onNavigate: (screen: Screen) => void, onPaymentComplete: () => void }) {
  const handlePayment = () => {
    onPaymentComplete();
  ...
  return (
    <div className="flex flex-col min-h-full pb-24 bg-white">
      <div className="px-4 pt-6 pb-2">
        ...
        <p className="text-slate-500 text-sm">Revise os serviços concluídos e agilize o fluxo de caixa.</p>
      </div>
      ...
      <div className="flex flex-col gap-1 mt-2">
        <div className="bg-white mx-4 rounded-2xl p-4 border border-blue-500/10 shadow-sm">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex flex-col justify-center flex-1">
              ...
              <p className="text-slate-500 text-sm">Higienização Completa + Cera</p>
              <p className="text-slate-400 text-xs mt-1 flex items-center gap-1">
                <Clock className="w-3 h-3" /> Concluído há 12 min
              </p>
            </div>
          </div>
        ...
        <div className="px-4 mt-8">
          <h3 className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-3 px-1">Próximos na Fila</h3>
          <QueueItem 
